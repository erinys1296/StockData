...
tab1, tab2, tab3 = st.tabs(["主圖", "支撐壓力","即時盤"])
...
with tab3:
    ...
    FutureData = get_future_raw_data(startFuture,endFuture)
    ...
    fig3_1.update_xaxes(
        ...
        rangebreaks=[
            dict(bounds=[6, 8], pattern="hour"),
            dict(bounds=['sat', 'mon']),
            dict(bounds=[13, 15], pattern="hour"),# hide weekends, eg. hide sat to before mon
            dict(values=[str(holiday) for holiday in holidf[~(holidf["說明"].str.contains('開始交易') | holidf["說明"].str.contains('最後交易'))]["日期"].values]+['2023-08-03'])
        ],
                    row = 1, 
                    col = 1
