...
tab1, tab2, tab3 = st.tabs(["主圖", "上下極限","選擇權三大法人籌碼"])
...
with tab2:
    ...
    #Final60Tdata.loc[Final60Tdata.dateonly ==datetime(2025, 4,7, 0, 0),'dateonly'] = datetime(2025, 4, 2, 0, 0)
    ...
    df_300.loc[df_300.dateonly ==datetime(2025, 3,2, 0, 0),'dateonly'] = datetime(2025, 2, 27, 0, 0)
    #df_300.loc[df_300.dateonly ==datetime(2025, 4,7, 0, 0),'dateonly'] = datetime(2025, 4, 2, 0, 0)
...
with tab3:
    st.header("選擇權三大法人籌碼")
    connection1 = sqlite3.connect('選擇權分析資料.sqlite3')
    df_options_futures_bs = pd.read_sql("select distinct * from options_futures_bs", connection1)
    df_options_futures_daygap = pd.read_sql("select distinct * from df_options_futures_daygap", connection1)
    putcallsum_sep = pd.read_sql("select distinct * from putcallsum_sep", connection1)
    ...
    with col4:
        ...
        df_cost = pd.read_sql("select distinct * from df_cost", connection1)
        ...
        df_option_limit = pd.read_sql("select distinct * from df_option_limit", connection1)
